...
%  VMIN^2<= sum(repmat(speye(2), M, 1).*reshape(permute(Wnn2Phi,[2 1 3 4]),2,2*M).',2)<=VMAX^2;
% VMIN^2<= sum(repmat(speye(1), M, 1).*reshape(permute(Wnn1Phi,[2 1 3 4]),1,1*M).',2)<=VMAX^2;
...
W2Phi(:,:,ll,:)>=0;
        ...
        WnmTilde(Branch.Phases{l}, Branch.Phases{l},:)=Wnm1Phi(:,:,ll,:); 
         ...
         conj(premute(WnmTilde(Branch.Phases{l},Branch.Phases{l},:),[2 1 3])), WmmTilde(Branch.Phases{l},Branch.Phases{l},:)];
W1Phi(:,:,ll,:)>=0;
