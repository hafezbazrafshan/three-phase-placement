...
%                         Branch.Wye3PhiTaps=[Branch.Wye3PhiTaps,[0; 0 ;0 ]]; % nominal taps
%                         TapA=10;
%                         TapB=8;
%                         TapC=11;
TapA=0;
                ...
                case 'OpenDelta'
                     Branch.RegulatorTypes=[Branch.RegulatorTypes;'OpenDelta'];
                        ...
                        Branch.OpenDeltaTaps=[Branch.OpenDeltaTaps,[7; 4 ]];
                         TapAB=7;
                        TapCB=4;
                        ArAB=1-0.00625*TapAB;
...
Bus.PhasesIndex=cell(NBuses,1); % modification!
...
%   Bus.PhasesIndex{ii}=(ii-1)*3+PhaseSet;
    Bus.SLoad{ii}=zeros(length(PhaseSet),1);
    MissingPhases=setdiff([1;2;3],PhaseSet);
    
    
     if ii==1
             Bus.PhasesIndex{ii}=(1:length(PhaseSet)).';
         else
             Bus.PhasesIndex{ii}=Bus.PhasesIndex{ii-1}(end)+(1:length(PhaseSet)).';
     end
